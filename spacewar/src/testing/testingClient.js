...
function Game() {
  ...
  useEffect(() => {
    ...
    // 2) 서버가 보내주는 'updateGameState' 이벤트 수신
    newSocket.on("updateGameState", (data) => {
      //setShipPos(data.players.shipPos);
      //setPlayerPos(data.players.playerPos);
      //setWeaponAngle(data.players.weaponAngle);
  ...
  }, [shipPos, playerPos, weaponAngle, socket]);
  ...
  const shoot = () => {
    ...
    const angleRad = (weaponAngle * Math.PI) / 180;
    const newBullet = {
      x: shipPos.x + 150 * Math.cos(angleRad), // 대충 우주선 앞쪽
      y: shipPos.y + 150 * Math.sin(angleRad),
      angleRad,
      speed: 10,
      radius: 5,
    };
